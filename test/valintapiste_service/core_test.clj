...
(deftest valintapiste-tests
  (let [mockedMongo (fn [hakuOID hakukohdeOID] ["testi-hakemus-1" "1.2.3.4"])]
    ...
    (testing "Test GET /haku/.../hakukohde/... returns empty pistetiedot"
      (let [response ((app (fn [hakuOID hakukohdeOID] []) datasource) (-> (mock/request :get  "/api/haku/1.2.3.4/hakukohde/1.2.3.4")))
            body     (parse-body (:body response))]
        (is (= (:status response) 200))
        (is (= body []))))

    (testing "Test GET /haku/.../hakukohde/... returns one pistetieto"
      (let [response ((app (fn [hakuOID hakukohdeOID] ["1.2.3.4"]) datasource) (-> (mock/request :get  "/api/haku/1.2.3.4/hakukohde/1.2.3.4")))
            body     (parse-body (:body response))]
        (is (= (:status response) 200))
